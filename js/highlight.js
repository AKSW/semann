 ...
 */
			...
			function rangy_serialize() {
				...
				var page = currentPageNo(obj.node);
			...
			function currentPageNo(element) { //returns the current page number that the given element belongs to
				var classname, index;
			...
			window.onload = function() { //rangy related objects that need initialisation
				rangy.init();
				cssApplier = rangy.createCssClassApplier("highlight", {normalize: true});
				highlightRanges = new Array();
			};
			
			function deserializeArray(array) { //deserialise "&rangyFragment" parameter value in the URI, return rangy range array
				var rangy_base = document.getElementById('viewer');
			...
			function rangy_highlight(rangyFragments) { //given rangy fragments (eg. ["0/3/3/1:0,0/3/3/1:9","0/1/3/1:17,0/1/3/1:21"]), apply highlights
				highlightRanges = deserializeArray(rangyFragments);
			...
			function getURLParameters(sURL, paramName) { //filters out givenURI parameter value from the sURL
			    if (sURL.indexOf("?") > 0)
			    {
